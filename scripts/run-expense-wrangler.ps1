# Expense Wrangler scheduled wrapper (Close-Out Gate hardened 2026-07-03; shared mechanism scripts/lib/close-out.ps1)
...
$log = "outputs\logs\expense-wrangler.log"
...
if (-not (Test-AlexQuotaGate -Log $log -Project 'expenses')) { exit 0 }

$out = ''
try {
    # Model: Sonnet-4-6 (cost cut, Shaheen 2026-07-16).
    $out = (& "$env:APPDATA\npm\claude.ps1" --model claude-sonnet-4-6 -p "Run /expense-wrangler. $AlexVerdictInstruction" --dangerously-skip-permissions 2>&1 | Out-String)
    $code = $LASTEXITCODE
} catch {
...
Invoke-CloseOutCheck -Out $out -Code $code -Log $log -Project 'expenses'
