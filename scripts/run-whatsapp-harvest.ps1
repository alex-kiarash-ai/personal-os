# WhatsApp Harvest scheduled wrapper (Close-Out Gate hardened 2026-07-03; shared mechanism scripts/lib/close-out.ps1)
# Task is DISABLED (paused 2026-06-18); hardened for consistency so it is safe if re-enabled.
# No run_status tile (Project ''); failure is detected, logged FAILED, and exits 1.
...
$log = "outputs\logs\whatsapp-harvest.log"
...
try {
    ...
    $out = (& "$env:APPDATA\npm\claude.ps1" --model claude-sonnet-4-6 -p "Run /whatsapp-harvest. $AlexVerdictInstruction" --dangerously-skip-permissions 2>&1 | Out-String)
    $code = $LASTEXITCODE
} catch {
