...
- **Alex HQ metrics push** (build #16 contract, work/16-alex-hq/CLAUDE.md). Push BOTH events in one call: `ytd_income_kr` (the big number) and `next_booking` (the HQ tile's sub line, added 2026-07-06 — `value_text` is guest + dates like `"a guest · sample dates"`, or exactly `"no booking"` when nothing upcoming; never fabricate one). Never let a push failure fail the run; never print or log the token:
  `curl -s -m 10 -X POST https://n8n.shaheenkiarash.com/webhook/alex-push -H "Content-Type: application/json" -H "X-Alex-Token: $(cat work/16-alex-hq/config/alex-hq-token.txt)" -d '{"events":[{"project":"airbnb","metric_key":"ytd_income_kr","value_num":{YTD payout total},"headline":"{next arrival / incoming payout / pending request}","status":"{amber if a pending request risks expiring, else green}"},{"project":"airbnb","metric_key":"next_booking","value_text":"{Guest · Mon D-D}|no booking","headline":"next check-in {date} · confirmed {date}","status":"green"}]}' || true`
