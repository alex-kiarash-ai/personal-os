...
- **HQ inbox:** `node scripts/lib/close-out.mjs hq-get --url https://n8n.shaheenkiarash.com/webhook/alex-inbox` (the helper holds the token inside Node so it never reaches argv; never printed). If `count_new` > 0, file the notes per work/16-alex-hq/CLAUDE.md "Inbox Contract" (voice → scp + local Whisper first) and mark them; report "HQ notes: N filed". Unreachable → one line, continue.
