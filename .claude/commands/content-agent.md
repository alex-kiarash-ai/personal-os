...
4. Brand + Soul Pre-Flight Gate (voice output): print the pre-flight line, re-read the loaded soul core (the LOCKED EPISODE TEMPLATE is PINNED into the card since 2026-08-16, Shaheen's "pin the template" - the earlier full-file exception is retired). Pull full soul.md only when drafting needs a register the card lacks (e.g. his older LinkedIn public-register edit examples, 06-13 + 07-16 entries). Draft 2-3 posts on the /post-episode quality bar (locked EPISODE TEMPLATE, ~150 words, one thought per line, Built with: footer, 3-4 niche hashtags). English only. Real numbers only.
