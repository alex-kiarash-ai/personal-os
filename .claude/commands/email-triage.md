...
1b. **HQ inbox check (two-way notes, build #16; runbook = work/16-alex-hq/CLAUDE.md "Inbox Contract").** GET `/webhook/alex-inbox` via `node scripts/lib/close-out.mjs hq-get --url https://n8n.shaheenkiarash.com/webhook/alex-inbox` (never a raw curl: the helper keeps the token inside Node, out of argv) with the X-Alex-Token header (token file work/16-alex-hq/config/alex-hq-token.txt, never printed). `count_new` 0 or unreachable → continue, never fail the run. Voice notes: scp audio from n8n:/opt/alex-inbox-audio/, transcribe with local Whisper, file per the vault protocols, POST `/webhook/alex-inbox-mark`, delete audio both ends. Report "HQ notes: N filed → destinations".
...
- **Alex HQ metrics push** (build #16 contract). Push `act_now` AND a GREEN `run_status` (green clears any stale red from a prior failed run). Never let a push failure fail the run; never print or log the token:
  `node scripts/lib/close-out.mjs hq-push --project alex --events '{"events":[{"project":"email-triage","metric_key":"act_now","value_num":{act now count},"headline":"run {N}: {act} act now, {read} read later","status":"{amber if act_now > 0, else green}"},{"project":"email-triage","metric_key":"run_status","value_num":0,"headline":"clean run {N}","status":"green"}]}' --log outputs/logs/hq-push.log`
