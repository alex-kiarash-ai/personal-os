...
- **Alex HQ metrics push** (build #16 contract, work/16-alex-hq/CLAUDE.md). Push `act_now` AND a GREEN `run_status` (the green clears any stale red the wrapper pushed on a prior failed run - the health board must distinguish "died at the scheduler" from "ran clean"; without the green push a one-off session-limit fail stays red forever, which happened 07-03→07-05). Never let a push failure fail the run; never print or log the token:
