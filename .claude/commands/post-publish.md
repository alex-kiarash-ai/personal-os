...
2. None found → report "no approved episode to stage" and stop.
3. Create the episode's Drive folder "Building Alex"/episode-NN-slug/ and upload the TEXT ONLY: post.txt (the full page-body text, copy-paste ready). Images are manual, Shaheen adds his own image to the folder. Then read-back verify the file and the row (the workflow does this with two verify nodes).
