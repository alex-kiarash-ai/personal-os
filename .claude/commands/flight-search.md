# /flight-search - Cheapest + Best Flights Across Three Live Sources

Spec: work/24-flight-search/CLAUDE.md (read it first; it defines the full runtime flow, the hard constraints, and the source lanes). On-demand. Alex is the master agent orchestrating **three live flight lanes** in parallel, with a fourth slot (Skyscanner) left **pluggable but UNWIRED by decision**.
...
3. **Fan out to ALL THREE LIVE lanes in parallel, every time, same criteria** (load deferred MCP tools via ToolSearch first): Kiwi `mcp__claude_ai_Kiwi_com__search-flight`; Turkish `mcp__turkish-airlines__search_flights` (+ `search_inbound_flights`, `find_flight_price_calendar`); Google Flights MCP. adults=1, cabin=economy. Flex dates -> expand the per-leg window, use each source's native calendar/range where it has one, keep the cheapest per option. No smart pruning. **The Skyscanner slot is UNWIRED by decision: do not call it and do NOT report it as a down lane.** An unwired-by-choice slot is not an outage, and printing "Skyscanner: unavailable" on every single search would train the reader to skip the exact line that has to carry a real outage.
4. **A LIVE lane that fails -> report it, do not hide it, do not block the rest.** "Kiwi: unavailable" under the table. Never fabricate. This line must stay rare enough to mean something.
...
## Hard constraints
Fresh every search (no cached prices) · all three LIVE lanes always (the Skyscanner slot is unwired by decision, never called, never reported as down) · graceful degradation on a down lane · cheapest-only on cross-source duplicates · currency as returned (no FX) · markdown booking links · 30-min memory then expire.
