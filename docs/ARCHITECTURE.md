<!-- GENERATED FILE - do not hand-edit. Source: templates/architecture.template.md + CLAUDE.md. Regenerate: node scripts/generate-alex.js. Generated 2026-09-16. -->
...
7. **The plain-English guide** (`Desktop\01 Projects\Alex\Story & Guides\Alex-Plain-English-Guide.docx`, STANDING ORDER + ROLE, Shaheen 2026-07-15): ANY system-related change MUST update it - its home section + a dated row in the section-12 running-changes table (`Date | What changed`); redraw the section-2 system-map table only when a whole LAYER moves. Write in the guide's own plain-English register (honest, present tense, short sentences, no em-dashes). **Append the running-changes row with `node scripts/identity-docs.js add-row --doc guide --date <YYYY-MM-DD> --text "..."`** (A17-T-07, 2026-09-11): it is the ONE writer for both identity docs and it takes the shared `alex-surfaces` lock, which is the third lock caller P-56 asked for and had nothing to attach to while every session wrote its own ad-hoc python-docx script. It places prose and refuses to invent it, so the home-section edit is still yours to write under the gate. Anchors are named by SECTION + TABLE HEADING, never T-numbers (remap history: annex). **Both files MOVED 2026-08-21** from `Desktop\Alex Project\Alex Presentation\files` to `Desktop\01 Projects\Alex\Story & Guides` in a Desktop reorganisation; the old path is dead. If neither is where this says, search for the filename before assuming it is gone.
8. **The technical master reference** (`Desktop\01 Projects\Alex\Story & Guides\ALEX-OS-master.md`, STANDING ORDER + ROLE, Shaheen 2026-07-16; local-only, OUTSIDE the repo): the living technical ground-truth mirror. Edit the numbered section where the change lands AND append a dated §11 running-changes line with `node scripts/identity-docs.js add-row --doc master --date <YYYY-MM-DD> --text "..."` (same writer, same lock). Register: verified ground truth, code/API/scheduler-accurate.
...
10. **The Alex Kit** (`Desktop/alex-kit`, added 2026-08-21). **Three** family members run installed copies (+1 asked, Mac proven), so a universal improvement here is an improvement they are missing. After a real system change, run the `/port-to-kit` triage test: tied to a project the Kit does not ship, or needing infrastructure it lacks, or naming Shaheen and his lanes = DROP or GENERALIZE; a universal rule, gate, skill or checker = PORT. Either port it or write the NOT PORTED row with the reason in `vault/projects/alex-kit/ported.md`; a deliberate skip nobody recorded reads as an oversight later. **The trap worth naming: their `soul.md` is gitignored**, so git delivers tracked files only and any identity or voice change ALSO needs a `scripts/migrations/NNN-*.js` in the Kit, or it silently never arrives. Delivery on their side is one double-click of `Update-Alex.cmd`, never a re-install.
...
4. **The 21:30 nightly sweep is the BACKSTOP, not the workflow.** `scripts/git-backup.sh` commits + pushes whatever branch the machine is standing on, GREEN/RED to HQ, so a day where a human saved nothing still ends saved. **It commits MODIFICATIONS to already-tracked files only** (A16-T-02, 2026-09-11): a NEW path under a work/NN root, `scripts/`, `docs/` or `brand/` is held unstaged and named in an AMBER line until a session `git add`s it deliberately, because a personal document whose sensitivity is in its prose passes every scanner and a sweep must never be the thing that PUBLISHES a file. A held file is still in that night's encrypted blob. On a properly-committed day it finds nothing, which is the healthy state. Because main is protected, **the working checkout never parks on main** - it sits on the current work branch, so the sweep always has a pushable home.
...
**IDENTITY DOCS are their own trigger (2026-08-28, applying self-review Proposal D of 2026-08-03 after the same miss recurred on the same file).** Any write into `Desktop\01 Projects\Alex\Story & Guides\Alex-Plain-English-Guide.docx` or `ALEX-OS-master.md` runs the gate, **including a running-changes append**. Items 7 and 8 already call these identity-carrying, but a gate fires reliably only when its OWN trigger list names the surface. Why it took two misses, and the corrupted path found while applying it: docs/constitution-annex/standing-orders-history.md.
...
- **No-dash sanitizer is REAL CODE** in the **QA + Fill Templates** nodes (corrected 2026-09-11, stress-test A15-T-09: this said "Parse Writer" and "en-dash → comma", and BOTH were wrong against the live workflows. Measured on #03: em-dash → `', '`, en-dash → `'-'`, and a second helper collapses either to `'-'` where a numeric range must survive). A doc that names the wrong node sends the next person to edit a node that does nothing, and they will find no dash code there and conclude it was never built. Re-test after any prose-model change = run the Writer Voice Eval (`grMqmGzzbTXTEdKr`, 6 seeded cases, must be 6/6).
...
- **docs/ = human-readable layer (added 2026-07-02):** docs/projects/ (per-project what/why/connections, non-technical voice) and docs/n8n/{workflow}/ (node-by-node README per running workflow, plus a live JSON export that is LOCAL-ONLY: `.gitignore` carries `docs/n8n/*/*.json`, so a fresh clone gets the READMEs and no exports, and the 09-04 audit found a second export beside the one narrow rule that had been tracked). When a project or live workflow changes for real, its docs/ file is part of the Change Propagation surface; refresh the n8n export in the same session.
...
- **The layer under every gate in this file is a MODEL, not a rule (stated 2026-09-10, stress-test A14-T14).** In auto mode the harness reviews each action against the conversation before running it. That is a judgement, and judgements are not guarantees. Three consequences worth holding: a boundary set in conversation ("don't push today") is read from context, is NOT stored anywhere, and is LOST when compaction drops the message that stated it; a rule written in this file is followed because a model read it and agreed, which is reliable but not mechanical; and only a `deny` or `ask` entry in `.claude/settings.json` is enforced by something that cannot be talked out of it. Write the rule here for the reasoning; put it in the permission set when the cost of one miss is unacceptable.
...
| 03 | /application-engine | PARKED (revisit 2026-11-01) | n8n Tue+Thu 15:00 + watch 8:30 | Job pipeline, Power BI track: source, score, gate, draft, render every Tue & Thu; also an MCP server. PARKED 2026-09-14 on Shaheen's word, deactivated on the box the same minute and read back. Resume context: the Anthropic spend was stopped deliberately because the output did not justify it, not an outage. To resume, reactivate the workflow and top up; nothing else was changed. | work/03-application-engine - vault/projects/job-pipeline/status.md |
...
| 14 | (no command) | PARKED (revisit 2026-11-01) | n8n Tue+Thu 15:30 | Job pipeline, AI track: clone of #03 with the AI CV + a recalibrated career-changer gate. PARKED 2026-09-14 on Shaheen's word, deactivated on the box the same minute and read back. Resume context: the Anthropic spend was stopped deliberately because the output did not justify it, not an outage. To resume, reactivate the workflow and top up; nothing else was changed. | work/14-ai-application-engine - vault/projects/ai-job-pipeline/status.md |
...
| 32 | (no command) | PARKED (revisit 2026-11-01) | n8n Tue & Thu 15:43 (drain + draft) | Standalone company-portal job lane, STAGE 2 of 2: drains the queue #31 banks and runs its OWN cloned Match/Gate/Writer/Render pipeline to review-ready drafts. Split from the scanner 2026-07-28. PARKED 2026-09-14 on Shaheen's word, deactivated on the box the same minute and read back. Resume context: the Anthropic spend was stopped deliberately because the output did not justify it, not an outage. To resume, reactivate the workflow and top up; nothing else was changed. | work/32-portal-application-engine - vault/projects/portal-application-engine/status.md |
...
| 34 | (no command) | LIVE | n8n weekdays 06:30 (collect + score) | Daily job-search collector for the Power BI lane: the LinkedIn guest endpoints plus six free public remote boards into one deduped row shape, scored against the BI profile, written to a Google Sheet Shaheen reviews. The source contract lives in work/34-job-search-bi/config/sources.json and is SHARED with #35, so the two lanes can never drift a field name. It collects and scores, it never applies. | work/34-job-search-bi - vault/projects/job-search-bi/status.md |
| 35 | (no command) | LIVE | n8n weekdays 06:45 (collect + score) | Daily job-search collector for the AI Automation lane: the SAME source contract and the same row shape as #34, scored against the AI lane profile with its own keyword set and its own sheet tab. Split from #34 so each lane carries its own n8n id and its own cron and comes under V6 leg (c) plus the daily active-flag watcher. It collects and scores, it never applies. | work/35-job-search-ai - vault/projects/job-search-ai/status.md |
| 36 | (no command) | DORMANT (revisit 2026-10-01) | n8n weekdays 07:15 (draft CV + cover letter) | The writing half of the job search. One workflow reads BOTH job-search spreadsheets (#34 BI and #35 AI) at 07:15, and for each row worth applying to it produces a tailored one-page CV and a cover letter into a Drive folder, then writes an applications row, a jobs.status cell and a writer_runs row per lane and reads every one of them back. The CV cannot contain a rewritten sentence by construction: the selector emits block ids and the assembler emits master strings, so an invented id is a refusal rather than a paragraph. It writes documents. It never applies, never emails, never opens a browser. DORMANT waiting on one named dependency: seat 8, the letter eval at 6/6 plus the first-fire drill at cap 2, and Shaheen's go to activate. | work/36-job-application-writer - vault/projects/job-application-writer/status.md |
| - | Voice | DORMANT (revisit 2026-11-05) | Ctrl+Alt+D dictate + the native /voice HOLD mode; the Stop/Notification speech hooks are NOT installed (removed 2026-08-05 with the lane); v2 loop on-demand | Voice v3 'ride the official surface' (research run 22, built 2026-07-12): two-way voice INSIDE the interactive Claude Code session. In: native /voice HOLD dictation (EN/SV, free, review-then-Enter - autoSubmit OFF by design vs acceptEdits) + Ctrl+Alt+D local-whisper dictate lane for AR/SV/EN (types into the prompt, never presses Enter). Out: Stop-hook Edge-TTS->SAPI never-mute speech, gated on outputs/voice/voice-on.flag ('voice on/off' to Alex). $0/mo, no long-lived audio process. v2 open-mic loop (alex_voice.py) stays the on-demand walk-around tool. | work/voice/README.md |
...
3. The commit-time guard [[me/NEVER-TOUCH]] (V10) is safety-critical, not advisory; the pre-commit hook also runs a staged-content `gitleaks` scan (fail-CLOSED throughout since 2026-08-28: a found secret blocks, a gitleaks that errors blocks, an ABSENT gitleaks blocks, and as of 2026-09-10 one that will not print a version blocks too, because a stub named `gitleaks` that exits 0 passed the leg as clean. `ALEX_ALLOW_NO_GITLEAKS=1` is the loud, deliberate override. This line said fail-OPEN for 13 days after the code stopped doing that, A12-T12).
...
- **Toggle:** when Shaheen says "voice on" / "voice off", create/delete `outputs/voice/voice-on.flag` (the Stop and Notification speech hooks are NOT installed in `.claude/settings.json` today - both keys were removed on 2026-08-05 when the voice lane went DORMANT, so the flag is read by `work/voice/v3` only; re-adding them is part of re-arming that lane). Buttons: `work/voice/v3/voice-on.cmd` / `voice-off.cmd`.
...
- **A. Every run:** (A1) blocked/degraded runs record BLOCKED/PARTIAL + reason, push RED, fabricate nothing, flag every unverified value; (A2) log.md entry written; (A3) status.md last_run + outcome updated; (A4) Alex HQ run_status pushed; (A5) temp artifacts deleted, only finals remain; (A6) every deliverable file written to outputs/ this run has a ledger row: `node scripts/outputs-ledger.js add --project {name} --path {path} --desc "{what it is}"` (an IDENTITY-carrying deliverable - .pdf/.docx/.pptx/.png/.jpg/.svg/.html - also requires `--grader PASS|FAIL|SKIPPED`, plus `--grader-note` for the last two; A01-T-06, 2026-09-11: the blind grader is advisory by Shaheen’s choice and fired on 2 of 11 identity days, so the row must SAY what happened even when the answer is SKIPPED, rather than leaving an absence nobody can see later) (the nightly reconcile heals misses within a day, but the row written NOW carries a real description instead of a filename skeleton).
...
- **Verify-after-write (STANDING ORDER, Shaheen 2026-07-12, D1 of the upgrade P0):** any write that mutates an external system (n8n REST, Notion, Google Sheets, systemd timers, HQ data tables, scp targets) must be followed IN THE SAME RUN by a read-back of the mutated fields, hard-failing (or logging RED) on mismatch. "It returned 200" is not verification. Born from the 2026-07-10 silent dual-engine deactivation (n8n PUT dropped the active flag; see error-log). Close-Out B enforces it per run. **The ONE named exemption (stated 2026-09-10, A16-T11, because it was already true in code and nowhere in the rule): the heartbeat `run_status` push.** `common.sh:hq_push` returns 0 by design and drains the response body without reading it, because an undeliverable heartbeat is a logged problem and never a failed run: a dashboard that cannot be reached must not turn a healthy job red. That carve-out covers the HEARTBEAT only. Every DATA write to an HQ table, and every other external write named above, is not exempt and must read back.
...
- **One repo-surface mutator at a time (STANDING RULE, stress-test fix F-08, 2026-07-25).** Two parallel sessions must never both run a system-mutating batch. **SIX writers** now take ONE shared cross-process lock (`scripts/lib/write-lock.js`, an atomic mkdir mutex with a 30-min stale-steal): `generate-alex.js`, `lib/build-soul-core.js`, `run-vault-index.sh`, `skills-installer.js`, `skills-park.js` and `status-rotate.js`. It began as two, the generator and the installer, because both write the same `CLAUDE.md` (the generator its routing region, the installer its auto-skills region), and this sentence still said two after four more joined (corrected 2026-09-10, stress-test A14-T10). A count that under-reports its own holders invites the next writer to skip the lock, which is exactly how the rule erodes. The generator FAILS LOUD if it cannot acquire (you asked for surfaces to be regenerated; silently doing nothing would be a lie); the installer DEFERS (its weekly run is opportunistic). This is the machine behind a real incident: on 2026-07-20 three back-to-back "download this skill" requests had parallel sessions race `skills-lock.json`, and one rewrite mis-attributed one skill and deleted another. Take the same lock around any new tooling that rewrites a generated surface.
