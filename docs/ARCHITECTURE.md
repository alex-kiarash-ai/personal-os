<!-- GENERATED FILE - do not hand-edit. Source: templates/architecture.template.md + CLAUDE.md. Regenerate: node scripts/generate-alex.js. Generated 2026-09-14. -->
...
| 34 | (no command) | DORMANT (revisit 2026-10-15) | n8n weekdays 06:30 (collect + score) | Daily job-search collector for the Power BI lane: the LinkedIn guest endpoints plus six free public remote boards into one deduped row shape, scored against the BI profile, written to a Google Sheet Shaheen reviews. The source contract lives in work/34-job-search-bi/config/sources.json and is SHARED with #35, so the two lanes can never drift a field name. It collects and scores, it never applies. Waiting on: Anthropic credits (scoring returns credit-exhausted, so no row is ever written) and Shaheen's explicit go to free an n8n slot and activate. Built and verified; the workflow exists and is deliberately inactive. | work/34-job-search-bi - vault/projects/job-search-bi/status.md |
| 35 | (no command) | DORMANT (revisit 2026-10-15) | n8n weekdays 06:45 (collect + score) | Daily job-search collector for the AI Automation lane: the SAME source contract and the same row shape as #34, scored against the AI lane profile with its own keyword set and its own sheet tab. Split from #34 so each lane carries its own n8n id and its own cron and comes under V6 leg (c) plus the daily active-flag watcher. It collects and scores, it never applies. Waiting on: Anthropic credits (scoring returns credit-exhausted, so no row is ever written) and Shaheen's explicit go to free an n8n slot and activate. Built and verified; the workflow exists and is deliberately inactive. | work/35-job-search-ai - vault/projects/job-search-ai/status.md |
