<!-- GENERATED FILE - do not hand-edit. Source: templates/getting-started.template.md + system/manifest.json + scheduler/schedule.md + CLAUDE.md. Regenerate: node scripts/generate-alex.js. Generated 2026-09-16. -->
...
## 4. The automations (35 registered, non-retired)
...
- **03 Application Engine (BI)** (PARKED, revisit 2026-11-01; trigger: n8n Tue+Thu 15:00 + watch 8:30) - Job pipeline, Power BI track: source, score, gate, draft, render every Tue & Thu; also an MCP server. PARKED 2026-09-14 on Shaheen's word, deactivated on the box the same minute and read back. Resume context: the Anthropic spend was stopped deliberately because the output did not justify it, not an outage. To resume, reactivate the workflow and top up; nothing else was changed.
...
- **14 AI Application Engine** (PARKED, revisit 2026-11-01; trigger: n8n Tue+Thu 15:30) - Job pipeline, AI track: clone of #03 with the AI CV + a recalibrated career-changer gate. PARKED 2026-09-14 on Shaheen's word, deactivated on the box the same minute and read back. Resume context: the Anthropic spend was stopped deliberately because the output did not justify it, not an outage. To resume, reactivate the workflow and top up; nothing else was changed.
...
- **32 Portal Application Engine** (PARKED, revisit 2026-11-01; trigger: n8n Tue & Thu 15:43 (drain + draft)) - Standalone company-portal job lane, STAGE 2 of 2: drains the queue #31 banks and runs its OWN cloned Match/Gate/Writer/Render pipeline to review-ready drafts. Split from the scanner 2026-07-28. PARKED 2026-09-14 on Shaheen's word, deactivated on the box the same minute and read back. Resume context: the Anthropic spend was stopped deliberately because the output did not justify it, not an outage. To resume, reactivate the workflow and top up; nothing else was changed.
...
- **34 Job Search BI** (LIVE; trigger: n8n weekdays 06:30 (collect + score)) - Daily job-search collector for the Power BI lane: the LinkedIn guest endpoints plus six free public remote boards into one deduped row shape, scored against the BI profile, written to a Google Sheet Shaheen reviews. The source contract lives in work/34-job-search-bi/config/sources.json and is SHARED with #35, so the two lanes can never drift a field name. It collects and scores, it never applies.
- **35 Job Search AI** (LIVE; trigger: n8n weekdays 06:45 (collect + score)) - Daily job-search collector for the AI Automation lane: the SAME source contract and the same row shape as #34, scored against the AI lane profile with its own keyword set and its own sheet tab. Split from #34 so each lane carries its own n8n id and its own cron and comes under V6 leg (c) plus the daily active-flag watcher. It collects and scores, it never applies.
- **36 Job Application Writer** (DORMANT, revisit 2026-10-01; trigger: n8n weekdays 07:15 (draft CV + cover letter)) - The writing half of the job search. One workflow reads BOTH job-search spreadsheets (#34 BI and #35 AI) at 07:15, and for each row worth applying to it produces a tailored one-page CV and a cover letter into a Drive folder, then writes an applications row, a jobs.status cell and a writer_runs row per lane and reads every one of them back. The CV cannot contain a rewritten sentence by construction: the selector emits block ids and the assembler emits master strings, so an invented id is a refusal rather than a paragraph. It writes documents. It never applies, never emails, never opens a browser. DORMANT waiting on one named dependency: seat 8, the letter eval at 6/6 plus the first-fire drill at cap 2, and Shaheen's go to activate.
...
| Job Search BI (#34) - box-side n8n cron, NOT a local timer | n8n workflow `oSVDR2WjkZnjovCP` (no local wrapper, no claude call, zero local tokens) | **weekdays 06:30 Stockholm** (`30 6 * * 1-5`), **ARMED 2026-09-14**. The workflow is active; the first scheduled fire is the next weekday 06:30. |
| Job Search AI (#35) - box-side n8n cron, NOT a local timer | n8n workflow `TNvg3zbOzd3rGr9T` (no local wrapper, no claude call, zero local tokens) | **weekdays 06:45 Stockholm** (`45 6 * * 1-5`), **ARMED 2026-09-14**. The workflow is active; it executed once by hand first (execution 5249), so its first scheduled run is not its first run. |
