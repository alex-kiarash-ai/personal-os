...
**Workflow ID:** `9XuIEfxS71DEetVR` · **Runs:** Tuesday & Thursday 15:00 Stockholm (cron `0 15 * * 2,4`; retimed 2026-07-24, was every-72h 07:00) · **Nodes:** 38 (32 after the 07-28 simplify, +6 in the 2026-08-07 repair) · **Model:** `claude-opus-5` on the Match/reasoning call, `claude-sonnet-5` on the Writer/prose call (2026-08-07; SUPERSEDES the 2026-07-27 Moonshot `kimi-k3` move, which SUPERSEDED claude-opus-4-8). Contract: `system/manifest.json` -> `meta.model_routing` · **Export in this folder:** workflow.json, re-pulled live 2026-08-07 at `versionId 96240ce7-ec6c-4290-9914-86cabaabbf04` / `updatedAt 2026-08-07T08:47:23.184Z` / 38 nodes / active:true. That is the graph described below.
...
1. **Model swap (2026-07-27):** all four job lanes moved off Claude/`claude-opus-4-8` to Moonshot `kimi-k3` at `reasoning_effort:'high'`. **SUPERSEDED 2026-08-07: the lanes run `claude-opus-5` (Match) + `claude-sonnet-5` (Writer) on Anthropic `/v1/messages` today; the Moonshot endpoints and the `Kimi K3` credential described in this paragraph are history, kept because the node NAMES and shapes below still match. Live contract: `system/manifest.json` -> `meta.model_routing`.** Provider swap, not a string swap - the two model nodes (`Claude Match+Research`, `Claude Writer`) now call `https://api.moonshot.ai/v1/chat/completions` via the `Kimi K3 (Moonshot header)` credential with OpenAI-format bodies, `max_tokens` 16384, a 10-min HTTP timeout. The node names still say "Claude"; only the endpoint and body changed.
...
- **Build Match Request** - packs the job posting plus Shaheen's master CV into a scoring request (Anthropic `/v1/messages` body with a cached system block since 2026-08-07; was an OpenAI-format Moonshot body during the kimi-k3 period).
- **Claude Match+Research** - the first of only two model calls; it calls `api.anthropic.com/v1/messages` on `claude-opus-5` (it called `api.moonshot.ai` between 2026-07-27 and 2026-08-07, which is why older notes say otherwise). The Parse node filters `content[]` for `type==='text'`, because adaptive thinking puts a THINKING block first and a `content[0].text` reader fails every job silently. Returns a fit score, an interest score, the detected work condition, the target role, and the reasoning. Batched with retry.
...
- **Build Writer Request** - packs the job + CV + the injected soul.md voice block into the writing brief. Anthropic `/v1/messages` body, `max_tokens` 16384 (was a Moonshot body during the 2026-07-27 to 08-07 kimi-k3 period).
- **Claude Writer** - writes the tailored CV content and cover letter for this specific job. Calls `api.anthropic.com/v1/messages` on `claude-sonnet-5` (was `api.moonshot.ai` 2026-07-27 to 2026-08-07); batched + retry.
