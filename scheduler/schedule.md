...
- Description: Level-triggered deterministic consistency sweep. Validates the WHOLE system against system/manifest.json (30 checks, C1-C31 with C16 retired: quad completeness, orphans x3, wiki-link resolution, routing rows, scheduler↔Windows-Task-Scheduler timers, dependent staleness, log monotonicity, manifest hash self-check, index↔disk, outputs naming, first-fire aging, passphrase attestation, PAT expiry, skills-symlink restore guard, machine timezone vs travel-state, narrative numbers-drift, backup destinations, facts-ledger doc drift, soul-corpus monotonicity, soul-core freshness, status byte budget, inbound mail channels, log tail ordering, soul-core byte budget, user-scope skill inventory, hook liveness, code-map freshness, task-completion dead-man switch). Detects, never repairs. Exit 0 clean / 2 drift / 1 error. Writes vault/projects/recovery/last-sweep.md (Monday brief reads it) + pushes recovery/integrity to Alex HQ (green clean / amber drift). Plan: vault/projects/recovery/recovery-layer-plan.md.
- Added: 2026-07-04
...
- **Leg 2, execution health (added 2026-08-05, pen-test P-02):** the flag leg only proves a workflow is switched ON. It now also makes one `/executions?limit=250` read and asserts per governed workflow that **(a)** the last execution did not error, and **(b)** a success exists inside the window its declared `n8n_cron` implies (daily -> 2d, weekly-ish -> 8d, both doubled for slack). Leg (b) applies only to lanes that declare a cron; #16/#17 are webhook-driven and their silence is owned by the HQ self-heal `health-source-stalled` probe. Executions unreadable = leg skipped, never a false RED. Exit 1 now also means "a governed workflow's last run errored, or it has gone quiet past its cadence". Born from 2026-08-04/05, when #03 and #14 both errored while this watcher, V6 and the weekly sweep all read green - all three inspect configuration, so they were not independent evidence.
- Added: 2026-07-16
...
- Frequency: **weekdays 06:30 Stockholm** (`30 6 * * 1-5`), **ARMED 2026-09-14**. The workflow is active; the first scheduled fire is the next weekday 06:30.
- Status: **LIVE** (activated 2026-09-14 on Shaheen's word, after the Anthropic top-up and four manual runs). Activation was done with a full before-and-after census of every active workflow, not on the 200: on 2026-07-27 activating a lane silently deactivated two live engines and both calls returned 200. Nothing was evicted this time; the box went 14 to 16 active.
- Description: Collects every Power BI shaped posting from the last 24 hours across the LinkedIn guest endpoints and six free public remote boards, folds them into one deduped row shape, scores each against the BI profile, and writes a Google Sheet Shaheen opens in the morning. It collects and scores, it never applies. **The declared cron stays load-bearing while the lane is off:** V6 leg (c) compares this exact string to the live trigger on every validate run, so a hand edit in the n8n editor is caught even with nothing running. The 08:10 active-flag watcher governs this lane again now the row is LIVE, which is what you want while it is running: it takes every LIVE row carrying an n8n id and reports one that has gone inactive. It was deliberately DORMANT between 2026-09-14 afternoon and evening precisely because that rule has no carve-out for a workflow that is off on purpose.
...
- Frequency: **weekdays 06:45 Stockholm** (`45 6 * * 1-5`), **ARMED 2026-09-14**. The workflow is active; it executed once by hand first (execution 5249), so its first scheduled run is not its first run.
- Status: **LIVE** (activated 2026-09-14, same census, nothing evicted). Repointed off the `jobs_test` tab first: execution 5249 wrote 16 rows and read all 16 back, which is the precondition the test tab existed to wait for. Activating with that key still present would have filled a test tab every weekday and never advanced the window.
