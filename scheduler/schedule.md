...
- Description: Level-triggered deterministic consistency sweep. Validates the WHOLE system against system/manifest.json (29 checks, C1-C30 with C16 retired: quad completeness, orphans x3, wiki-link resolution, routing rows, scheduler↔systemd timers, dependent staleness, log monotonicity, manifest hash self-check, index↔disk, outputs naming, first-fire aging, passphrase attestation, PAT expiry, skills-symlink restore guard, machine timezone vs travel-state, narrative numbers-drift, backup destinations, facts-ledger doc drift, soul-corpus monotonicity, soul-core freshness, status byte budget, inbound mail channels, log tail ordering, soul-core byte budget, user-scope skill inventory, hook liveness, code-map freshness). Detects, never repairs. Exit 0 clean / 2 drift / 1 error. Writes vault/projects/recovery/last-sweep.md (Monday brief reads it) + pushes recovery/integrity to Alex HQ (green clean / amber drift). Plan: vault/projects/recovery/recovery-layer-plan.md.
- Added: 2026-07-04
