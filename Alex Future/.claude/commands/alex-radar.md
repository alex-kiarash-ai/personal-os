...
3. **Fetch (free feeds only in Phase 0-1).** Load WebSearch/WebFetch. Pull new items from sources.md (HN, GitHub release feeds, Product Hunt, OpenAI/Google RSS, MCP registry) since last run. Include the **self-watch feeds** (Claude Code / MCP servers / n8n releases + Anthropic news; see sources.md "Self-watch") - own-stack releases are scored like any item, `Improves = Alex`. Include the **community feeds** (r/mcp, r/ClaudeAI, n8n forum; sources.md "Community feeds") under the cold-profile cap: max 2 community items presented per sweep, in-community echo = 1 corroboration, high-confidence needs a non-community source.
