---
name: no-ai-slop
description: Detect and name AI-slop patterns in a draft (colon reveals, faux-insight setups, importance puffery, weasel attribution, synonym cycling, metadiscourse, fake-profound kickers), or edit a draft to remove them while preserving the writer's voice. Use when a draft reads as AI, when auditing prose before it ships, or when the user asks whether writing sounds like AI. In Alex, DETECT mode is the primary use; soul.md always outranks this file on any conflict.
---
...
this block and the rewritten frontmatter description (precedent: agent-reach 2026-07-29,
karpathy-guidelines 2026-07-28, the 9 curated description rewrites 2026-07-11).
...
1. EM DASHES: this file says "1-2 are fine in longer drafts". WRONG HERE. Shaheen's rule is
   ZERO em dashes and ZERO en dashes, always, every lane, master text included (standing
   order 2026-08-20, which reversed an earlier carve-out after dashes shipped in a live CV).
   It is enforced in real code: the n8n Parse Writer sanitizer, the build-cv-master.py guard,
   and NEGATIVE marks in all three resync engines. Never relax it on this file's authority.

2. BUILDING ALEX EPISODES (#12) ARE EXEMPT from four of its bans. The locked ~150-word
   episode template REQUIRES the binary-contrast turn ("But the interesting part isn't X.
   It's Y"), one short contrast pair for punch, one ownable thesis line, and one-thought-per-line
   spacing. This file calls all four slop (binary contrasts, fake-profound kickers, dramatic
   fragmentation, formatting slop). Inside a #12 episode the template wins and this file is
   ignored on those four. Everywhere else (cover letters, emails, CV prose, slide copy, docs)
   its bans apply normally. Source of the template: soul.md episode template + work/12 HARD RULES.

3. VOICE COMES FROM THE CORPUS, NOT FROM THE DRAFT. This file infers voice from the single
   draft in front of it ("identify 3-5 voice signals"). Alex does not work that way: the voice
   source is soul.md My Words, and the Brand + Soul Pre-Flight Gate requires reading it before
   generating. Read the corpus first; use this file only to name patterns in what comes out.

4. ESL PHRASING IS THE SIGNAL, NOT AN ERROR. This file never mentions it, and a generic sharp
   editor "fixes" it. Shaheen's detection-proofing rule 1 is absolute: dropped articles, dropped
   -s and -ed, run-ons, "she stop talking", "I born and raised", "bc", "frozed" are PRESERVED.
   Never correct his grammar into polished English on this file's authority. That correction is
   the single fastest way to make his text read as machine-written.

Also: this file's "ask the user to paste a draft" and "ask one question about audience" steps are
fine interactively, but never block a headless run on them; a scheduled run uses what it has.
========================================================================== -->
