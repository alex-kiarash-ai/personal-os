...
## Notion Integration
**Personal CRM** database under the Personal Ops System parent page. db_id `0da7196d-f52b-435b-8692-76dd75a0dc24`, data_source `746bc5bf-8ab3-4e34-911d-00b9d180e350`. Properties (live schema, verified 2026-07-10):
- **Name** (title)
...
3. **Score + status (deterministic + channel-aware, since 2026-07-10).** Read each contact's **Channel** (from the people-page `channel:` frontmatter, synced to the Notion `Channel` field; email / whatsapp / linkedin / in-person / mixed / unknown) and branch:
   ...
   - **Cadence circles (Dunbar tiers, Phase 1, 2026-07-25):** a contact's people-page frontmatter may carry `circle: A|B|C|X` - an explicit tier Shaheen sets. **A = monthly (30d), B = quarterly (90d), C = twice-yearly (180d), X = no cadence** (tracked, never listed as due). Precedence in `scripts/personal-crm-core.js`: a numeric `cadence.json` override (this Claude pass's computed value) > the circle tier > the channel/category default. The circle is ONLY the cadence; `last_contact` stays the recency source (email-triage maintains it from Gmail sent), so nothing is duplicated. The core emits a `state/relationships-block.md` (overdue circled contacts, capped at 5) consumed by #10.
