...
- **Scheduled (Phase 1, LIVE 2026-07-02):** weekly Stream B sweep, **Mondays 07:30** (slot confirmed by Shaheen 2026-07-02), Task Scheduler job `PersonalOS-alex-radar` → `scripts/run-alex-radar.sh` → headless `claude -p "Run /alex-radar --weekly"`, log in `outputs/logs/alex-radar.log`. Runs BEFORE the 08:00 [[projects/morning-brief/status|Morning Brief]], which surfaces the output as its **Radar section** (rule added to work/02-morning-brief/CLAUDE.md), not a separate ritual.
