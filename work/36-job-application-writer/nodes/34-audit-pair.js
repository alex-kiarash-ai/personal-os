...
function auditPair(pair, phase) {
  ...
  // --- A8. Every figure in the letter is approved, or sits inside a span the letter is allowed to
  ...
  if (research.hook && research.hook.quote) quoteSources.push(String(research.hook.quote));
  if (pair.site_text) quoteSources.push(String(pair.site_text));
  const allow = {};
  ...
  while ((sm = spanRe.exec(letter)) !== null) {
    ...
    const ok = quoteSources.some((s) => spanIsIn(span, s)) || (adText && spanIsIn(span, adText));
