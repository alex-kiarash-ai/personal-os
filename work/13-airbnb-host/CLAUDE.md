...
## Entry Points
- **Scheduled: monthly, the 24th at 10:00** - `PersonalOS-airbnb-host` runs `scripts/run-airbnb-host.sh` (pinned `--model claude-sonnet-4-6`, harvest passes `--headless`). *(Corrected 2026-07-29, architecture review: this bullet said "No separate schedule", contradicting both `system/manifest.json` and `scheduler/schedule.md`, which have carried the monthly job all along. The task is registered and Ready in Task Scheduler.)*
