...
## Purpose
Track Shaheen's Airbnb hosting of a studio apartment from a **local read-only Playwright harvest of his own logged-in host dashboard**, with the **Gmail feed as the fallback**, because Airbnb has no host API and no usable MCP (official API is partner-only; community MCPs are guest-side listing search only). *(Corrected 2026-07-29, architecture review: this paragraph said "entirely from the Gmail feed", which contradicted BOTH the manifest one-liner and the Data Access section directly below it in this same file. The browser harvest has been the primary source since Shaheen chose it on 2026-06-14, and `raw/reservations-*.json` files prove it runs; see the Data Access section, which was right all along.)* Two jobs: (1) **track + flag** - maintain an occupancy calendar, log every booking, and surface pending requests and unanswered guest messages into the [[morning-brief]]; (2) **income tracker** - parse payout emails into a Notion Bookings DB and a branded Excel income model with REAL formulas (occupancy %, nightly average, monthly net) that feeds [[me/situation]]. Knowledge lives at [[me/airbnb-studio]].

## Entry Points
- **Scheduled: monthly, the 24th at 10:00** - `PersonalOS-airbnb-host` runs `scripts/run-airbnb-host.ps1` (pinned `--model claude-sonnet-4-6`, harvest passes `--headless`). *(Corrected 2026-07-29, architecture review: this bullet said "No separate schedule", contradicting both `system/manifest.json` and `scheduler/schedule.md`, which have carried the monthly job all along. The task is registered and Ready in Task Scheduler.)*
- **Also folded into /morning-brief:** the daily brief already reads Gmail; it surfaces Airbnb pending requests (URGENT, they expire) and guest messages. That fold-in is in addition to the monthly job, not instead of it.
