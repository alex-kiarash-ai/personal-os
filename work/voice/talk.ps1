<#
  ...
  Say "hey jarvis" to start a turn; just talk over Alex to cut in; Ctrl-C or "goodbye Alex" to quit.
#>
$py = Join-Path $PSScriptRoot ".venv\Scripts\python.exe"
if (-not (Test-Path $py)) { $py = "python" }   # fall back to global if the venv is gone
Write-Host 'Starting Alex Voice v2... (say "hey jarvis", talk over Alex to interrupt, Ctrl-C to quit)' -ForegroundColor Cyan
