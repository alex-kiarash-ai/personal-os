...
if (sheetUsable) {
  if (fresh.length > CAP) {
    capped = fresh.length - CAP;
    out = fresh.slice(0, CAP);
  }
} else {
  out = [];
}
...
if (capped > 0) {
  warnings.push(
    'THE CAP BIT. ' + fresh.length + ' new job(s) survived and max_scored_per_run is ' + CAP + ', so ' + capped +
    ...
    ' rows this run wrote, and the next ' + CAP + ' come through. The backlog drains ' + CAP + ' a run. ' +
...
const report = {
  ...
  advance_last_run_at_why: !sheetUsable
    ...
    : (capped > 0
      ? 'the cap held back ' + capped + ' real row(s) that are written nowhere. Advancing the window would lose them permanently.'
...
module.exports = {
  ...
  notes: 'Refuses a failed sheet read rather than reading it as an empty sheet, collapses the same job arriving from several sources without ever merging fields, drops what is already in the tab, then applies max_scored_per_run newest first. A capped run reports that last_run_at must not advance.',
  parameters: {
