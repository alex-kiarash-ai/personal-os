...
- **Run:** n8n editor → open the workflow → **Execute workflow**. Do this after every `scripts/sync-soul-to-n8n.js --apply` (the eval is a sync-soul TARGET, so its writer stays in lockstep with the live engines).
...
- **First run (2026-07-07):** 4/6 pass. Caught 2/6 cover letters with dashes despite the no-dash rule and confirmed the live writer is `claude-sonnet-4-6` (the routing rule says gpt-4.1-mini - still to reconcile).
