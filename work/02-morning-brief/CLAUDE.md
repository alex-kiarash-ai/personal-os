...
## Notion Integration
**Daily Briefs** database under the Personal Ops System parent page:
...
**Mail-channel check (every brief, added 2026-08-23).** The daily half of recovery C25. Read `system/mail-channels.json`, and for each channel run its `gmail_query` (newest message only) to get the last arrival time. Write all of them to `work/18-recovery-layer/state/mail-channels-seen.json` as `{checked_at, channels:{<address>:{last_seen, last_from}}}` - that stamp is what lets the zero-token weekly C25 assert freshness without a Gmail call of its own, so **writing it is not optional even when nothing is wrong**. Then surface a line ONLY when a channel with a non-null `max_silence_days` has exceeded it: "No mail at <address> in N days (window Md) - the forward may be dead". Channels with `max_silence_days: null` are never aged (shaheen@ legitimately takes 3 messages in 3 months; the Cloudflare config leg is its only watcher, and inventing a window there would only cry wolf). **Why this is a brief job and not just a weekly one:** on 2026-08-23 the site's ONLY public contact address was found dead since ~2026-06-08 with nothing red anywhere - Cloudflare accepts mail for an address with no routing rule and silently drops it, so the sender sees success and the recipient sees nothing. Same doctrine as the Pipeline-health check above: the brief is the human notification surface, and a mail channel dying quietly is exactly the class it exists for.

**HQ inbox check (every brief, added 2026-07-02).** The Alex HQ PWA has a two-way notes card (typed + voice). Every brief run fetches `GET /webhook/alex-inbox` (X-Alex-Token) and files every `new` note per the standing vault protocols, transcribing voice notes with local Whisper first (full mechanics: work/16-alex-hq/CLAUDE.md "Inbox Contract" + step 5 in the command file, renumbered from 4b on 2026-07-28). Correction-type notes additionally route to #22 Teach-Alex (classified + appended to vault/projects/teach-alex/corrections-log.md; identity files confirm before applying). **CRM-action notes are applied same-day (added 2026-07-10):** a note matching a CRM contact with the deterministic verbs `talked <name> [date]` (set `last_contact` on the people page + Notion row), `snooze <name> to <date>` (set Follow-Up Date), or `close <name>` (Status→Cold) is executed here so it does not wait for Monday's #05 run - it reuses the same trivial field-write pattern as the Life Ops write-back. The Gmail-touching CRM verbs `send`/`draft` are NOT applied here; they stay behind #05's draft gate. The brief gets a short **"Notes you dropped"** block showing each note → destination. Unreachable inbox = one FYI line, never a failed brief.

**Provenance on retrieved content (P7.2, run-47 merged plan, 2026-08-23).** When a brief line quotes or leans on something RETRIEVED rather than observed this run, say where it came from. Three tags, the same vocabulary `recall-inject.js` already stamps on every injected block, so one convention covers both surfaces:

- `[fact]` - from `system/recall/facts.db`, machine-harvested from a structured source. Trustworthy by construction.
- `[vault]` - from a vault page. Human or agent prose, unreviewed, and it may itself be quoting inbound content.
- `[lesson]` - a past Close-Out lesson. Model-emitted, unreviewed.

**Observed content needs NO tag** and must not get one: mail read this run, today's calendar, an n8n alert row, a live API read. Those are what the brief IS. Tagging everything would make the tags invisible, which defeats them.
