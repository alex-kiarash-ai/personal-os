...
5. **Render verification for any deck, not optional (same run):** print the PDF, assert page count equals slide count, extract text PER PAGE and assert a known footer or page-marker string appears on every one, then rasterise every page and LOOK at them. **Two re-verification traps, both measured twice on run 58 (2026-09-02):** headless `chrome --print-to-pdf` can RETURN before the PDF hits disk, so a verify step that runs next reads the stale copy and passes - assert the PDF mtime is newer than the HTML before trusting any re-check; and rasterised pages re-read from the SAME filenames can serve a stale image - render re-checks to fresh filenames. Also: when a fix adds a new visual layer, re-run the brand accent scan on the new layer specifically; run 58's double-orange violation was fixed in the base deck and immediately recommitted by the diagram layer. Two slides overflowed and silently dropped their footer on this run; the deck looked fine to the eye and only the per-page extraction caught it. Same lesson as the 2026-07-18 clipped CV, one layer up.
6. **Four mechanics for an ANIMATED deck, all measured on run 59 (the technical deck with a moving background).** (a) **A `prefers-reduced-motion` guard must FORCE the end states, never just `animation:none`.** Entrance classes have `opacity:0` as their base state under `body:not(.static)`, so a blanket kill leaves every animated element permanently invisible; the guard has to set `opacity:1;transform:none;filter:none` on those classes and only then kill the ambient loops. (b) **A blurred moving backdrop is the new grain overlay for PDF weight:** three 90px-blur layers per page rasterised to 11.0 MB across 12 pages, and swapping them for flat radial gradients on the STATIC path only brought it to 4.05 MB with the on-screen blur untouched. Same shape as the run-49 grain lesson, one layer up: pay for the effect on screen, never in the shared file. (c) **The backdrop lives INSIDE each page**, same rule as the chrome, or the print path paints it on page one only. (d) **Small-colour accents recur in every new layer:** the run-58 never-small-orange rule was broken twice again here by a 14px and a 15px uppercase label, so the accent scan must check SIZE as well as count, and orange belongs only in large display text.
7. **Rendering a real mermaid diagram with no mermaid CLI (run 59).** `npx mmdc` is not installed and the `mermaid.esm.min.mjs` bundle is chunked and unusable standalone; the self-contained UMD `mermaid.min.js` works. Render it in a local harness with headless Chrome `--dump-dom`, extract the SVG, inline it, and ship the `.mmd` as the source artifact. Three faults, all invisible to assertions and caught only by LOOKING at the PNG: `htmlLabels:false` collapses every second `<br/>` line to dots; **rewriting mermaid's own stylesheet by splitting the string on `}` corrupts it** and every edge paints as a filled white blob, so overrides must be APPEND-ONLY; and label text clips unless the harness `body` carries the same `font-family` mermaid measures with. Put the single brand accent on one node with a native `classDef` rather than post-processing the SVG.
