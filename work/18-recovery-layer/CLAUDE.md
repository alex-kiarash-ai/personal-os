...
- **Manifest** `manifest.json` - since 2026-07-06 (audit steps 3+5) **THE project registry**, not just the checker's input. Per project: num, name, title, state (LIVE/ON-DEMAND/EVENT/DORMANT/PARKED/RETIRED + revisit dates), trigger, one_liner, docs, schedule_jobs[], hq_project, work_dir, commands[], status_md, cadence_days, n8n id + `meta.unnumbered` (voice, alex-costs, modeling) + the allowlists. Edited BY HAND when anything project-level changes; then run **`scripts/generate-surfaces.ps1`**, which regenerates the root CLAUDE.md routing table AND the docs/projects/README table between markers (never hand-edit those). /new writes its registry entry first. This is the source of truth the sweep validates disk against AND the source the human-facing tables are generated from - one edit, no propagation drift.
...
3. **orphan work folders** - every `work/` dir (not just `NN-*`) is a manifest project or in the `meta.known_work_folders` allowlist (`voice`, the live TTS hook); catches a rogue non-numbered folder.
