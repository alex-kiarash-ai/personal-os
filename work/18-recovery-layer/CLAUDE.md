...
- **Still remaining:** (b) C6 basename fallback resolves `[[x/status]]` via any `status.md`, under-reporting path drift — for path-style links require the full relpath (deferred: changes findings); (d) implement the design's named **index.md ↔ disk** check; (f) C8 mtime staleness goes silent after a git clone resets mtimes (inherent).
