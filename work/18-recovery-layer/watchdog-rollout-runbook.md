...
> **PLATFORM NOTE (2026-08-25, powershell-branch reconciliation):** this runbook was written for the
> PowerShell wrappers. The system now runs bash wrappers, and the watchdog was ported as
> `run_with_watchdog` in `scripts/lib/common.sh` (GNU `timeout` process-group kill, `STALLED` flag,
> `CODE=124`; sets `$OUT`/`$CODE` like `alex_claude`). The rollout below is UNCHANGED in intent -
> one wrapper at a time, per-wrapper verification, the two [SHAHEEN] decision points stand - but the
> per-wrapper edit is now `alex_claude ...` -> `run_with_watchdog 25 "$CLAUDE" ...` in each
> `scripts/run-*.sh`, and every `Invoke-AlexWithWatchdog`/`close-out.ps1` reference below reads as
> its bash twin. Still DEFERRED; still recalled by "wire the watchdog".
